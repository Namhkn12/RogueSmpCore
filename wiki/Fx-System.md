...
- **`FxTransform.at(Location)` lấy cả `yaw` lẫn `pitch`** (yaw rồi tới pitch, cả 2 đều đảo dấu — cùng convention với `ParticleUtils.spawnShape` ở package `particle`), nên 1 shape hướng (`LineShape`, `HelixShape`) neo vào `Location` sẽ trỏ đúng hướng nhìn thật, kể cả nghiêng lên/xuống. Với vector hướng bất kỳ có sẵn (không đi qua `Location`, vd. hướng tính từ 2 điểm start/end như trong `DaggerThrow.spawnTracer`) thì dựng thẳng `Quaternionf().rotationTo(...)` từ vector đó vẫn đúng/gọn hơn — không cần quy đổi ngược ra yaw/pitch trước.
