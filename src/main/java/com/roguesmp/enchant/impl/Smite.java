...
public class Smite implements SmpEnchant {
    ...
    @Override public void onDamageEntity(DamageEvent event, int level, @NotNull SmpPlayer player) {
        Entity target = event.getVictim();
        if (EntityUtils.isUndead(target)) {
